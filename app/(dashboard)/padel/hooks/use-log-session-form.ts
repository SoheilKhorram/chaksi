...
export function useLogSessionForm(settings: PadelSettings, onSuccess?: () => void) {
  ...
  const [type, setType] = useState<'game' | 'training'>('training')
  const [isCustomPrice, setIsCustomPrice] = useState(false)
  const [customPrice, setCustomPrice] = useState('')
  const [extraItems, setExtraItems] = useState<ExtraItemForm[]>([])
  const [sendToPartner, setSendToPartner] = useState(settings.sendTrainingToPartner ?? false)
  ...
  const resetForm = () => {
    ...
    setCustomPrice('')
    setExtraItems([])
    setSendToPartner(settings.sendTrainingToPartner ?? false)
    setIsPaid(false)
    setError(null)
    setSuccess(false)
  }
