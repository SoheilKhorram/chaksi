...
function Tabs({
  ...
  const activeTab = controlledValue !== undefined ? controlledValue : localActiveTab
  ...
  const layoutId = React.useMemo(() => `tabs-active-${id}`, [id])
...
function TabsTrigger({
  ...
  const isActive = context ? context.activeTab === value : false
    ...
    >
      {isActive && context && (
        <motion.span
          ...
          className="absolute inset-0 bg-background dark:bg-input/30 dark:border dark:border-input rounded-md shadow-sm -z-10 group-data-[variant=line]/tabs-list:hidden"
